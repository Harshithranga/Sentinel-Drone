...
class Edrone():
   ...
   def __init__(self):
       ...
       self.setpoint = [[0, 2, 23], [0, 0, 23], [2, 2, 23], [-2, 2, 23], [-2, -2, 23], [2, -2, 23], [2, 0, 23], [0, 0, 23]]
       ...
       self.Kp = [2.01, 41.94, 40.8] #final settings [pitch, roll, throttle]
       self.Ki = [0.0014, 0, 0]
       self.Kd = [673.8, 602.7, 706.5]
       self.PID = [0.0, 0.0, 0.0]

       #self.Kp = [0, 0, 0]  # final settings [pitch, roll, throttle]
       #self.Ki = [0, 0, 0]
       #self.Kd = [0, 0, 0]
       #self.PID = [0.0, 0.0, 0.0]       # -----------------------Add other required variables for pid here ----------------------------------------------
       self.error = [0, 0, 0]
       self.prev_err = [0, 0, 0]
       self.diff_err = [0, 0, 0]
       self.sum_err = [0, 0, 0]
   ...
   def pid(self):
       ...
       while j<8:
           ...
           print(f'setpoint:{self.setpoint[j]}  error{self.error}')
           for x in range(3):
           ...
           if math.dist(self.drone_position, self.setpoint[j])<=0.34 and abs(self.setpoint[j][0]-self.drone_position[0])<=0.2 and abs(self.setpoint[j][1]-self.drone_position[1])<=0.2 and abs(self.setpoint[j][2]-self.drone_position[2])<=0.2:
               j+=1
               #rospy.sleep(10.)
           else:
               pass
...
#use the pid tuner, not the bracket values
